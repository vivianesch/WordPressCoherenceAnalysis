...
#Functions
    ...
    .Top.15.GroupO = function(x,y,f){
      z = subset(x, y %in% f) 
      z<-table(z$Keywords,z$Owner)
      d<-dim(z)
      w<-rowSums(z[1:d[1],1:d[2]])
      w<-as.data.frame(as.table(w)) # Transform to a data frame
      w<- w[order(w$Freq, decreasing = TRUE),] #Ranking
...
#1.1 Analyse Keywords used for Active reporters
    ...
    z = subset(Reporter_Keywords, Reporter_Keywords$Reporter %in% AR)
    
    .Unianalysis(z$Keywords)
...
#2. OWNERS AND KEYWORDS 
    ...
    # 10 top Keywords used by Active Group
    Most_Key_AO<-.Top.15.GroupO(Owner_Keywords,Owner_Keywords$Owner,AO) 
    
    #2.2 Analyse Keywords used for Median reporters
    MO<-.List.f.filter(MedianOwners)
    # 10 top Keywords used by Median Group
    Most_Key_MO<-.Top.15.GroupO(Owner_Keywords,Owner_Keywords$Owner,MO) 
    
    #2.3 Analyse Keywords used for Alien Owner
    LO<-.List.f.filter(LessOwners)
    # 10 top Keywords used by Alien Group 
    Most_Key_LO<-.Top.15.GroupO(Owners_Keywords,Owner_Keywords$Owners,LO) 
    
    
#3. REPORTERS AND OWNERS GRAPHICS

    #Graphics of most frequent words of Reporters
    #Active Group
    ...
    #Graphics of most frequent words of Owners 
    #Active Group
